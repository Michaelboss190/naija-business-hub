...
export default function ForumPostPage() {
  ...
  const { data: post, isLoading } = useSupabaseQuery(
    ['forum-post', postSlug || ''],
    async function() {
      const result = await supabase.from('forum_posts').select('*, author:profiles(username, avatar_url, full_name), category:forum_categories(name, slug, color)').eq('slug', postSlug || '').maybeSingle()
      return result
    }
  )
  ...
  const { data: comments, refetch: refetchComments } = useSupabaseQuery(
    ...
    function() { return supabase.from('forum_comments').select('*, author:profiles(username, avatar_url, full_name)').eq('post_id', post?.id || '').is('parent_id', null).order('created_at', { ascending: true }) },
    { enabled: !!post?.id }
  )

  const { data: allReplies } = useSupabaseQuery(
    ['post-replies', post?.id || ''],
    function() { return supabase.from('forum_comments').select('*, author:profiles(username, avatar_url, full_name)').eq('post_id', post?.id || '').not('parent_id', 'is', null).order('created_at', { ascending: true }) },
    { enabled: !!post?.id }
  )

  const { data: userLike } = useSupabaseQuery(
    ['post-like', post?.id || '', profile?.id || ''],
    function() { return supabase.from('likes').select('id').eq('post_id', post?.id || '').eq('user_id', profile?.id || '').maybeSingle() },
  ...
  useEffect(function() {
    ...
    // Check if bookmarked
    if (post?.id && profile?.id) {
      supabase.from('bookmarks').select('id').eq('user_id', profile.id).eq('post_id', post.id).maybeSingle().then(function({ data }) {
  ...
  const handleBookmark = async () => {
    ...
    try {
      const { data: existing } = await supabase.from('bookmarks').select('id').eq('user_id', profile?.id).eq('post_id', post?.id).maybeSingle()
      if (existing) {
  ...
  if (!post) return (
    <div className="container-custom py-16 text-center">
      ...
      <Link to="/forum"><Button aria-label="Button">Back to Forum</Button></Link>
    </div>
  )
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <motion.article initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 p-6 md:p-8 mb-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
            <div className="flex items-center space-x-3">
              <div className="w-12 h-12 bg-gray-100 dark:bg-dark-700 rounded-full flex items-center justify-center overflow-hidden">
                {post.author?.avatar_url ? <img src={post.author.avatar_url} alt="Image" className="w-full h-full object-cover" /> : <span className="text-gray-600 dark:text-gray-400 font-semibold">{post.author?.full_name?.charAt(0)?.toUpperCase() || "U"}</span>}
              </div>
              <div>
                <div className="flex items-center space-x-2"><span className="font-semibold text-gray-900 dark:text-gray-100">{post.author?.full_name || 'Anonymous'}</span>{profile?.is_premium && <span className="text-yellow-500">⭐</span>}</div>
            ...
            {profile?.id === post.author_id && (
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <Button variant="ghost" size="sm" className="flex-1 sm:flex-none" aria-label="Button">Edit</Button>
                <Button variant="danger" size="sm" onClick={handleDelete} className="flex-1 sm:flex-none" aria-label="Button">Delete</Button>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center space-x-4 md:space-x-6 pt-6 border-t border-gray-100 dark:border-dark-700">
            <button onClick={handleLike} className={'flex items-center space-x-2 text-sm transition-colors ' + (userLike ? 'text-red-500' : 'text-gray-500 dark:text-gray-400 hover:text-red-500')} aria-label="Button">
            ...
            <button onClick={handleBookmark} isLoading={isBookmarking} className={'flex items-center space-x-2 text-sm transition-colors ' + (isBookmarked ? 'text-primary-600 dark:text-primary-400' : 'text-gray-500 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400')} aria-label="Button">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 p-6 md:p-8">
          ...
          {isAuthenticated ? (
            <div className="mb-8">
              ...
              <div className="flex justify-end mt-3"><Button onClick={function() { handleSubmitComment(null) }} isLoading={isSubmitting} disabled={!newComment.trim()} aria-label="Button">Post Comment</Button></div>
            </div>
          ) : (
            <div className="mb-8 p-4 bg-gray-50 dark:bg-dark-700 rounded-lg text-center"><p className="text-gray-600 dark:text-gray-400 mb-2">Please login to join the discussion</p><Link to="/login"><Button size="sm" aria-label="Button">Login</Button></Link></div>
          )}
          <div className="space-y-6">
                ...
                <div key={comment.id} className="border-b border-gray-100 dark:border-dark-700 last:border-0 pb-6 last:pb-0">
                  <div className="flex items-start space-x-3">
                    <div className="w-10 h-10 bg-gray-100 dark:bg-dark-700 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden">
                      {comment.author?.avatar_url ? <img src={comment.author.avatar_url} alt="Image" className="w-full h-full object-cover" /> : <span className="text-gray-600 dark:text-gray-400 font-semibold text-sm">{comment.author?.full_name?.charAt(0)?.toUpperCase() || "U"}</span>}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center space-x-2 mb-1"><span className="font-semibold text-gray-900 dark:text-gray-100">{comment.author?.full_name || 'Anonymous'}</span>{profile?.is_premium && <span className="text-yellow-500 text-sm">⭐</span>}<span className="text-sm text-gray-500 dark:text-gray-400">{formatTimeAgo(comment.created_at)}</span></div>
                      <p className="text-gray-700 dark:text-gray-300 mb-3">{comment.content}</p>
                      <div className="flex items-center space-x-4">
                        <button onClick={function() { setReplyTo(replyTo === comment.id ? null : comment.id) }} className="text-sm text-gray-500 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400" aria-label="Button">Reply</button>
                            ...
                            return (
                              <div key={reply.id} className="flex items-start space-x-3">
                                <div className="w-8 h-8 bg-gray-100 dark:bg-dark-700 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden">
                                  {reply.author?.avatar_url ? <img src={reply.author.avatar_url} alt="Image" className="w-full h-full object-cover" /> : <span className="text-gray-600 dark:text-gray-400 text-xs font-semibold">{reply.author?.full_name?.charAt(0)?.toUpperCase() || "U"}</span>}
                                </div>
                                <div><div className="flex items-center space-x-2 mb-1"><span className="font-semibold text-gray-900 dark:text-gray-100 text-sm">{reply.author?.full_name || 'Anonymous'}</span><span className="text-xs text-gray-500 dark:text-gray-400">{formatTimeAgo(reply.created_at)}</span></div><p className="text-gray-700 dark:text-gray-300 text-sm">{reply.content}</p></div>
                              </div>
                            )
                          })}
                        </div>
                      ...
                      {replyTo === comment.id && (
                        <div className="mt-4 pl-6">
                          ...
                          <div className="flex justify-end gap-2 mt-2"><Button variant="ghost" size="sm" onClick={function() { setReplyTo(null); setReplyContent('') }} aria-label="Button">Cancel</Button><Button size="sm" onClick={function() { handleSubmitComment(comment.id) }} isLoading={isSubmitting} disabled={!replyContent.trim()} aria-label="Button">Reply</Button></div>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            {(!comments || comments.length === 0) && <div className="text-center py-8 text-gray-500 dark:text-gray-400">No comments yet. Be the first!</div>}
          </div>
        </motion.div>
      </div>
    </div>
