  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <ConfirmDialog
        ...
        message="Are you absolutely sure you want to permanently delete your account? ALL your data including posts, messages, vendor listings, subscriptions, and more will be permanently erased. This action CANNOT be undone."
        confirmLabel="Delete My Account"
        variant="danger"
        onConfirm={handleDeleteAccount}
        onCancel={function() { setConfirmDeleteAccount(false) }}
        ...
        <div className="p-6 md:p-8">
          ...
          {activeTab === 'profile' && (
            <div className="space-y-6">
              ...
              <Button onClick={function() { navigate('/profile') }} variant="outline" aria-label="Button">Edit Profile</Button>
            </div>
          )}
          ...
          {activeTab === 'password' && (
            <form onSubmit={handleChangePassword} className="space-y-6 max-w-md">
              ...
              <Button type="submit" isLoading={isChangingPassword} aria-label="Button">Update Password</Button>
            </form>
          )}
          ...
          {activeTab === 'notifications' && (
            <div className="space-y-6 max-w-md">
              ...
              {[{ key: 'email', label: 'Email Notifications', desc: 'Receive notifications via email' }, { key: 'push', label: 'Push Notifications', desc: 'Receive push notifications' }, { key: 'in_app', label: 'In-App Notifications', desc: 'See notifications in the app' }, { key: 'marketing', label: 'Marketing Emails', desc: 'Receive tips and offers' }].map(function(item) {
                return (
                  <div key={item.key} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-dark-700 rounded-lg">
                    ...
                    <button type="button" onClick={function() { setNotifPrefs({ ...notifPrefs, [item.key]: !notifPrefs[item.key as keyof typeof notifPrefs] }) }} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${notifPrefs[item.key as keyof typeof notifPrefs] ? 'bg-primary-600' : 'bg-gray-300 dark:bg-dark-500'}`} aria-label="Button"><span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${notifPrefs[item.key as keyof typeof notifPrefs] ? 'translate-x-6' : 'translate-x-1'}`} /></button>
                  </div>
                )
              })}
              <Button onClick={handleSaveNotifications} aria-label="Button">Save Preferences</Button>
            </div>
          )}
          ...
          {activeTab === 'privacy' && (
            <div className="space-y-6 max-w-md">
              ...
              {[{ key: 'show_email', label: 'Show Email', desc: 'Display email on profile' }, { key: 'show_phone', label: 'Show Phone', desc: 'Display phone on profile' }, { key: 'show_location', label: 'Show Location', desc: 'Display location on profile' }].map(function(item) {
                return (
                  <div key={item.key} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-dark-700 rounded-lg">
                    <div><p className="font-medium text-gray-900 dark:text-gray-100">{item.label}</p><p className="text-sm text-gray-500 dark:text-gray-400">{item.desc}</p></div>
                    <button type="button" onClick={function() { setPrivacySettings({ ...privacySettings, [item.key]: !privacySettings[item.key as keyof typeof privacySettings] }) }} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${privacySettings[item.key as keyof typeof privacySettings] ? 'bg-primary-600' : 'bg-gray-300 dark:bg-dark-500'}`} aria-label="Button"><span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${privacySettings[item.key as keyof typeof privacySettings] ? 'translate-x-6' : 'translate-x-1'}`} /></button>
                  </div>
                )
              })}
              <Button onClick={handleSavePrivacy} aria-label="Button">Save Settings</Button>
            </div>
          )}
            ...
            <div className="space-y-6 max-w-md">
              ...
              <div className="p-6 bg-gray-50 dark:bg-dark-700 rounded-xl">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <button onClick={function() { setTheme('light') }} className={`p-4 rounded-xl border-2 text-center ${theme === 'light' ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20' : 'border-gray-200 dark:border-dark-600'}`} aria-label="Button">
                    <svg className="w-8 h-8 mx-auto mb-2 text-yellow-500" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4 8a4 4 0 11-8 0 4 4 0 018 0z" clipRule="evenodd" /></svg>
                  ...
                  <button onClick={function() { setTheme('dark') }} className={`p-4 rounded-xl border-2 text-center ${theme === 'dark' ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20' : 'border-gray-200 dark:border-dark-600'}`} aria-label="Button">
                  ...
                  <button onClick={function() { setTheme('system') }} className={`p-4 rounded-xl border-2 text-center ${theme === 'system' ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20' : 'border-gray-200 dark:border-dark-600'}`} aria-label="Button">
              ...
              {profile?.is_premium ? (
                <div className="p-6 bg-gradient-to-r from-primary-50 to-primary-100 dark:from-primary-900/20 dark:to-primary-800/20 rounded-xl border border-primary-200 dark:border-primary-800">
                  ...
                  <div className="flex gap-3">
                    <Button onClick={function() { navigate('/pricing') }} variant="outline" aria-label="Button">Manage Plan</Button>
                    <Button variant="danger" onClick={function() { setConfirmCancelSub(true) }} aria-label="Button">Cancel Subscription</Button>
                  </div>
                </div>
              ) : (
                <div className="p-6 bg-gray-50 dark:bg-dark-700 rounded-xl">
                  ...
                  <Button onClick={function() { navigate('/pricing') }} aria-label="Button">Upgrade to Premium</Button>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === 'danger' && (
            <div className="space-y-6">
              ...
              <div className="p-6 border-2 border-red-200 dark:border-red-900/50 rounded-xl bg-red-50/50 dark:bg-red-900/10">
                ...
                <Button variant="danger" onClick={function() { setConfirmDeleteAccount(true) }} isLoading={isDeleting} aria-label="Button">
