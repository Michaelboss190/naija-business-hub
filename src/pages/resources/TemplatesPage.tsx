...
export default function TemplatesPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      const { data: template } = await supabase.from('templates').select('file_url, thumbnail_url').eq('id', deleteTarget).single()
      // Delete file from storage
      if (template?.file_url) {
        const path = template.file_url.split('/').slice(-2).join('/')
        await supabase.storage.from('resources').remove([path])
      }
      if (template?.thumbnail_url) {
        const thumbPath = template.thumbnail_url.split('/').slice(-2).join('/')
        await supabase.storage.from('resources').remove([thumbPath])
      }
      await supabase.from('templates').delete().eq('id', deleteTarget)
  ...
  if (error) return <div className="container-custom py-8"><ErrorState title="Failed to load templates" onRetry={function(){refetch()}} /></div>

  return (
    <div className="container-custom py-8">
      <ConfirmDialog isOpen={!!deleteTarget} title="Delete Template" message="Are you sure you want to delete this template? This cannot be undone." confirmLabel="Delete" onConfirm={handleDelete} onCancel={function(){setDeleteTarget(null)}} />

      <PageHeader title="Business Templates" description="Download ready-to-use templates for your business" actions={isAuthenticated ? <Link to="/templates/new"><Button>+ Upload Template</Button></Link> : undefined} />
      ...
      {!profile?.is_premium && (
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="bg-gradient-to-r from-yellow-50 to-yellow-100 dark:from-yellow-900/20 dark:to-yellow-800/20 rounded-xl p-4 mb-6 text-center border border-yellow-200 dark:border-yellow-800">
          <p className="text-sm text-gray-700 dark:text-gray-300">⭐ <Link to="/pricing" className="text-primary-600 dark:text-primary-400 font-medium hover:underline">Upgrade to Premium (₦1,000/mo)</Link> to download templates.</p>
        </motion.div>
      )}
      ...
      {isLoading ? <GridSkeleton cols={3} rows={2} /> : templates && templates.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {templates.map(function(template: any, index: number) {
            const isOwner = profile?.id && template.author_id === profile?.id
            return (
              <motion.div key={template.id} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:index*0.05}} className="bg-white dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 hover:border-primary-300 dark:hover:border-primary-600 hover:shadow-lg dark:hover:shadow-black/20 transition-all p-4 md:p-6 relative">
                {isOwner && (
                  <div className="absolute top-3 right-3 flex gap-1">
                    <Link to={'/templates/' + template.slug + '/edit'} className="p-1.5 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg hover:bg-blue-200 dark:hover:bg-blue-900/50" title="Edit"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>
                    <button onClick={function(e){e.preventDefault();setDeleteTarget(template.id)}} className="p-1.5 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-200 dark:hover:bg-red-900/50" title="Delete"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between text-xs md:text-sm text-gray-500 dark:text-gray-400 mb-3"><span>{template.category}</span><span>{template.file_type?.toUpperCase()}</span></div>
      ...
      ) : <EmptyState icon="📋" title="No templates found" description={debouncedSearch?'No templates match your search.':'Templates will appear here.'} actionLabel={isAuthenticated?'Upload Template':undefined} actionLink={isAuthenticated?'/templates/new':undefined} />}
    </div>
  )
}