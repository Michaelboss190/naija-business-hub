...
export default function ResourcesPage() {
  ...
  return (
    <div className="container-custom py-8">
      ...
      <PageHeader title="Resource Library" description="Business guides, templates, videos, and tools" actions={isAuthenticated ? <Link to="/resources/new"><Button>+ Add Resource</Button></Link> : undefined} />
      ...
      {isLoading ? <GridSkeleton cols={3} rows={2} /> : resources && resources.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {resources.map(function(resource: any, index: number) {
            ...
            return (
              <motion.div key={resource.id} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:index*0.05}} className="relative">
                <Link to={'/resources/' + resource.slug} className="block bg-white dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 hover:border-primary-300 dark:hover:border-primary-600 hover:shadow-lg dark:hover:shadow-black/20 transition-all p-4 md:p-6 h-full">
                  {isOwner && (
                    <div className="absolute top-3 right-3 flex gap-1 z-10">
                      <button onClick={function(e){e.preventDefault();e.stopPropagation();setDeleteTarget(resource.id)}} className="p-1.5 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-200 dark:hover:bg-red-900/50" title="Delete"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                    </div>
                  )}
                  <div className="flex items-center justify-between mb-3 md:mb-4">
                    <span className="text-2xl">{typeIcons[resource.type]||'📄'}</span>
                    <div className="flex items-center gap-2">{resource.is_premium&&<span className="px-2 py-1 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 rounded-full text-xs">⭐ Premium</span>}<span className="px-2 py-1 bg-gray-100 dark:bg-dark-700 text-gray-600 dark:text-gray-400 rounded-full text-xs">{resource.type}</span></div>
                  </div>
                ...
                {resource.file_url && (
                  <div className="absolute bottom-3 right-3 z-10">
                    <Button size="sm" onClick={function(e){e.preventDefault();e.stopPropagation();handleDownload(resource)}} disabled={resource.is_premium && !profile?.is_premium}>
