...
export default function MasterclassPage() {
  ...
  if (!course) return <div className="container-custom py-16 text-center"><div className="text-6xl mb-4">🎓</div><h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">Course not found</h2><Link to="/masterclasses"><Button aria-label="Button">Back to Masterclasses</Button></Link></div>
        ...
        {showEditCourse && isOwner && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 p-6 mb-8">
            ...
            <form onSubmit={handleCourseSubmit(onSaveCourse)} className="space-y-4">
              ...
              <div className="flex justify-end gap-3"><Button variant="outline" type="button" onClick={function(){setShowEditCourse(false)}} aria-label="Button">Cancel</Button><Button variant="danger" type="button" onClick={function(){setConfirmDeleteCourse(true)}} aria-label="Button">Delete Course</Button><Button type="submit" isLoading={isSavingCourse} aria-label="Button">Save Changes</Button></div>
            </form>
          </motion.div>
        )}
            ...
            <div className="bg-white dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 p-6">
              {isOwner ? (
                <div className="text-center">
                  ...
                  <Button variant="outline" size="sm" onClick={function(){setShowEditCourse(true)}} className="w-full" aria-label="Button">Edit Course</Button>
                </div>
              ) : !profile?.is_premium ? (
                <div className="text-center"><span className="text-3xl mb-3 block">⭐</span><p className="text-gray-600 dark:text-gray-400 mb-4">Premium membership required</p><Link to="/pricing"><Button className="w-full" aria-label="Button">Upgrade - ₦1,000/mo</Button></Link></div>
              ) : isEnrolled ? (
                ...
                <Button onClick={handleEnroll} isLoading={isEnrolling} className="w-full" size="lg" aria-label="Button">Enroll Now - Start Learning</Button>
              )}
            </div>
          </div>
        </div>
